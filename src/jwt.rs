...
/// 1 hour
const JWT_EXPIRATION_SECONDS: u64 = 3600;
...
pub fn issue_jwt(subject: &str, secret_key: &[u8]) -> Result<String, String> {
    ...
    let expiration = match now
        .checked_add(Duration::from_secs(JWT_EXPIRATION_SECONDS))
...
mod tests {
    ...
    fn test_issue_and_validate() {
        ...
        let token: String = issue_jwt(subject, &secret)
            .expect("JWT issuance should succeed");

        let claims: Claims = validate_jwt(&token, &secret)
